		...
		case *ast.AddressOf:
			...
			*section = append(*section, byte(a0.Value))
			...
			case *ast.Identifier:
				...
				*section = append(*section, []byte{0xDE, 0xAD, 0xBE, 0xEF}...)
			default:
				return fmt.Errorf("mov expected argument #2 to be ADDRESS_OF[REGISTER], ADDRESS_OF[NUMBER_LITERAL], or ADDRESS_OF[IDENTIFIER] got ADDRESS_OF[%s]", value.String())
			}
		default:
			return fmt.Errorf("mov expected argument #2 to be REGISTER, NUMBER_LITERAL, IDENTIFIER, or ADDRESS_OF got %s", arg1.String())
		}
